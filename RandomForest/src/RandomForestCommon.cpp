...
EigenMat RandomForest::ReadFloatMatrix(const std::string& line, int height, int width) {
  EigenMat mat(height, width);
  std::stringstream ss(line);
  std::string value;
...
void RandomForest::SerializeMatrix(const EigenMat& data, const std::string& fileName) {
