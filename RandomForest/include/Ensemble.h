...
public:
  ...
  //    const EigenMat& data);
  std::vector<float> GetUnaryCostsFromMeanLabel(const EigenMat& labels);
  std::vector<float> GetUnaryCostsFromImpurity(const std::vector<NodeR*>& leaves);
  // takes labels (n_trees + 1) which include the mean as well as leaves (n_trees)
  // it returns n_trees costs after dicarding the mean label (the first one in labels)
  std::vector<float> GetUnaryCostsFromMeanAndImpurity(
      const EigenMat& labels,
      const std::vector<NodeR*>& leaves);
  
  // for each patch returns the distance from the corresponding area in the guessed_matte
  std::vector<float> GetUnaryCostsFromGuessedMatte(
  ...
  RowVector Test(const RowVector& x);
	EigenMat TestGetAll(const RowVector& x);
	std::vector<int> TestGetAllSamples(const RowVector& x);
  std::vector<NodeR*> GetAllTerminalNodes(const RowVector& x);
  
  void GetAllSamplesFromTerminalNodes(const RowVector& x, std::vector<int>* samples, std::vector<float>* impurities);

  // The meat of the class - training happens here.
  // Takes labels in original (pre-PCA) space and converts them to low-dim when
  // needed using passed in PCAWrapper object.
	void Train(
  ...
  void ReadEnsembleBin(std::string fileName);
	void WriteEnsemble(std::string fileName);
	void WriteEnsembleBin(std::string fileName);

  //// DBG
  //void WriteTreeNodes(int scale, const EigenMat& labels, const PCAWrapper& pcaw) {
  //  // DBG
